        ...
        {timeline.map((item) => (
          <Card key={item.name}>
            <CardContent className="pt-6">
              <time dateTime={item.dateTime} className="flex items-center text-sm font-semibold text-primary">
                <svg viewBox="0 0 4 4" className="mr-4 h-1 w-1 flex-none" aria-hidden="true">
                  <circle cx={2} cy={2} r={2} fill="currentColor" />
                </svg>
                {item.date}
              </time>
              <h3 className="mt-6 text-lg font-semibold tracking-tight text-foreground">{item.name}</h3>
